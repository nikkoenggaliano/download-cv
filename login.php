...
if(isset($_POST['user']) && isset($_POST['pass'])){
	$user  = $_POST['user'];
	$pass  = $_POST['pass'];
  $user = str_replace(array('=','OR', 'or', '--+', '-- -', '#', 'AND'), "", $user);
  $pass = str_replace(array('=','OR', 'or', '#', '--+', '-- -', '1'), "", $pass);
	$query = "SELECT * FROM user where user = '{$user}' and pass = '{$pass}'";
	$exe = $pdo->query($query);
	$ff = $exe->fetchColumn();

	if((int) $ff > 0 ){
    $cok = base64_encode('cv.pdf');
    $token = rand(1000,10000);
    $json = array("user" => "data", "role" => "user", "random" => md5($token));
    setcookie("auth", base64_encode(json_encode($json)));
		setcookie("PHPSECID", $cok);
		$_SESSION['user'] = $user;
...
<div class="center">
    ...
    <div class="face">
        ...
        <div class="nose"><svg width="38.161" height="22.03"><path d="M2.017 10.987Q-.563 7.513.157 4.754C.877 1.994 2.976.135 6.164.093 16.4-.04 22.293-.022 32.048.093c3.501.042 5.48 2.081 6.02 4.661q.54 2.579-2.051 6.233-8.612 10.979-16.664 11.043-8.053.063-17.336-11.043z" fill="#243946"></path></svg>
            <div
                class="glow"></div>
    </div>
    <div class="mouth"><svg class="smile" viewBox="-2 -2 84 23" width="84" height="23"><path d="M0 0c3.76 9.279 9.69 18.98 26.712 19.238 17.022.258 10.72.258 28 0S75.959 9.182 79.987.161" fill="none" stroke-width="3" stroke-linecap="square" stroke-miterlimit="3"></path></svg>
        <div
            class="mouth-hole"></div>
...
<form method="post">
<div class="login">
	<label><div class="fa fa-phone">
		
	</div>
	<input class="username" type="text" name="user" autocomplete="on" placeholder="Username"/>
</label>
<label>
	<div class="fa fa-commenting">
		
	</div><input class="password" type="password" name="pass" autocomplete="off" placeholder="Password"/>
</label>
...
<div class="footer">너가 그것을 번역하면 너는 바보 야.</div>
</body>
</html>